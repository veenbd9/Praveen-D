    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10 flex flex-col items-center">
          ...
          <button onClick={onViewTerms} className="absolute bottom-4 text-slate-400 hover:text-emerald-300 underline text-sm">Terms of Service</button>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    setResetError(error.message || 'Unable to send the password reset email.');
                  } finally {
                    setResetLoading(false);
                  }
                }}
                disabled={resetLoading}
                className="w-full text-center text-emerald-400 hover:text-emerald-300 underline text-xs font-bold disabled:opacity-50"
              >
                {resetLoading ? 'Sending reset link...' : 'Forgot password?'}
              </button>
              {resetRequested && <p className="text-center text-emerald-300 text-xs">If an account exists for this email, a password reset link has been sent.</p>}
              {resetError && <p className="text-center text-red-300 text-xs">{resetError}</p>}
          ...
          {authStep === 'otp' && (
            <form onSubmit={handleSubmitOtp} className="space-y-6 relative z-10 animate-fade-in">
              <div className="text-center mb-4">
                ...
                <p className="text-slate-500 text-[10px] font-bold uppercase mt-1 tracking-widest">Code dispatched to your registered email</p>
              </div>
